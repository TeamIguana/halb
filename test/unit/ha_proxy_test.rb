...
module Halb
  class HAProxyTest < Test::Unit::TestCase
    ...
    def test_remove_from_lb
      assert_match /\^\(pxname1\|pxname2\),.*UP/, @load_balancer.show_active_hosts_command
      @load_balancer.output[@load_balancer.show_active_hosts_command] = [still_in_balancer_response_for('host_to_remove_from_lb'), host_out_from_balancer_response]
    ...
    def test_put_in_lb
      @load_balancer.output[@load_balancer.show_active_hosts_command] = [host_out_from_balancer_response, still_in_balancer_response_for('host_to_put_in_lb')]
      @load_balancer.expects(:sleep).once
